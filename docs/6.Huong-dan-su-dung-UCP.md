...
- **Bước 3**: Phân quyền sử dụng UCP cho User
	...
	**Password**: hoangdh@mdt (Tùy chọn)
	...
	- Cuối cùng, tab **UCP**. Tab này cho phép chúng ta cài đặt cho user có được sử dụng UCP hay không! 
		...
		- **Miscellaneous**: Cấp cho user sử dụng extention
		
		![ucp-hoangdh](/images/ucp-2-8-2.png)
		
		**Chú ý**: Có thể cấp nhiều extention cho user.
		
		- **Call History**: Cho phép user xem lại lịch sử cuộc gọi của extention
		
		![ucp-hoangdh](/images/ucp-2-8-3.png)
		
		**Chú ý**: Có thể lịch sử cuộc gọi của các extention khác nhau, chọn thêm ở `CDR Access`. 
		
		- **Call Event Logging**: Cho phép user xem lại nhật ký cuộc gọi của extention
